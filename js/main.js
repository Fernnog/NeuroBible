...
// [NOVO] Readequação Dinâmica de Carga por Perfil com Varredura Futura
window.recalculateLoad = function() {
    let movedItems = 0;
    const todayStr = getLocalDateISO(new Date()); 
    
    // 1. Busca o limite correto do Perfil atual (Ex: Leve = 4)
    const limit = srsEngine.getDynamicLimit ? srsEngine.getDynamicLimit(appData) : 4;

    // 2. Cria um mapa de carga atualizado APENAS do presente/futuro
    const liveLoadMap = {};
    ...
    // 3. Varre os versículos para encontrar os que caem em dias estourados
    appData.verses.forEach(v => {
        // Cria uma cópia das datas originais para iterar com segurança
        const currentDates = [...v.dates];

        currentDates.forEach(d => {
            // Se a data é de hoje em diante E o dia está acima do seu limite, readequar
            if (d >= todayStr && liveLoadMap[d] > limit) {
                
                // Retira provisoriamente este versículo do dia estourado
                liveLoadMap[d]--;
                v.dates = v.dates.filter(existingDate => existingDate !== d);

                // Procura a próxima vaga a partir do dia seguinte
                let searchDateObj = new Date(d + 'T00:00:00');
                searchDateObj.setDate(searchDateObj.getDate() + 1);
                let slotFound = false;

                // Tenta realocar dentro de uma janela de 60 dias
                for(let i = 0; i < 60; i++) {
                    const isoDate = getLocalDateISO(searchDateObj);
                    if ((liveLoadMap[isoDate] || 0) < limit) {
                        // Vaga encontrada
                        v.dates.push(isoDate);
                        v.dates.sort();
                        liveLoadMap[isoDate] = (liveLoadMap[isoDate] || 0) + 1;
                        slotFound = true;
                        movedItems++;
                        break;
                    }
                ...
                // Proteção: Se não houver vaga em 60 dias, devolve ao dia original
                if (!slotFound) {
                    v.dates.push(d);
                    v.dates.sort();
                    liveLoadMap[d]++;
    ...
    if (movedItems > 0) {
        ...
        if (window.showToast) window.showToast("Carga já está equilibrada dentro dos limites.", "info");
    }
};
...
window.onload = function() {
    ...
    // --- NOVO: MELHORIA DE ARQUITETURA (VISIBILITY CHANGE) ---
    // Monitora a volta do app que estava em background (PWA behavior)
    document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') {
            console.log("[System] App retomado do background. Verificando estado do dia...");
            
            const currentTodayISO = getLocalDateISO(new Date());
            const lastLoginState = appData.stats ? appData.stats.lastLogin : null;
            const isDayChangeDetected = lastLoginState && lastLoginState !== currentTodayISO;
            
            // Reexecuta o sanity check para garantir integridade dos contadores de PWA inativo
            const backgroundDataFixed = runSanityCheck();
            if (backgroundDataFixed) saveToStorage(); 
            
            if (isDayChangeDetected) {
                uiDashboard.checkStreak(); 
                saveToStorage();
                if(window.showToast) window.showToast("☀️ Bom retorno! Contadores diários resetados.", "info");
            }
            
            // Força a re-renderização completa da interface garantindo que os dados atuais batam com o dia
            uiDashboard.updateTable();
            uiDashboard.updateRadar();
            uiDashboard.updatePacingUI();
            uiDashboard.renderDashboard();
