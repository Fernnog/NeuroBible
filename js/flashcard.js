...
export function startFlashcard(verseId) {
    ...
    // Força o início da trilha direto nas Iniciais (Stage 0)
    setCardStage(0); 
...
function renderCardContent(verse) {
    ...
    // Limpeza de estados e remoção de lógica mnemônica
    contentEl.classList.remove('blur-text');
    contentEl.style.display = 'block';

    if (cardStage.value === 0) {
        // Estágio de Letras Iniciais
...
export function advanceStage() {
    ...
    if (current === 0) {
        setCardStage(1); // Avança direto para lacunas
    }
