...
// [ATUALIZADO] Importando findNextLightDay e getDynamicLimit para uso na função de Resgate
...
export function updateRadar() {
    ...
    // [REFATORADO P3] Usando a função centralizada para evitar duplicação
    ...
    for (let i = 0; i < 63; i++) {
        ...
        const safeLimit = Math.max(1, Math.floor(maxLimit * 0.5)); // Limite para "leve"
...
export function renderDashboard() {
    ...
    // [INTEGRAÇÃO P1] Dispara a Inteligência Preditiva no ciclo de renderização
    renderPredictiveCard();
}

// [NOVA FUNCIONALIDADE - P1 & P2] Inteligência Preditiva de Sobrecarga
export function renderPredictiveCard() {
    const panel = document.getElementById('predictivePanel');
    if (!panel) return;

    // Obtém o limite da capacidade máxima de acordo com o perfil
    const limit = getDynamicLimit(appData); 
    // [REFATORAÇÃO P3] Usa a função centralizada do mapa de carga
    const loadMap = getCurrentLoadMap();
    ...
    // Escaneia os próximos 60 dias a partir de hoje
    for (let i = 0; i < 60; i++) { 
        ...
        if (load >= limit) {
            ...
            nextSafeDateISO = iso; // Primeiro dia com folga
            break;
        }
        currentDate.setDate(currentDate.getDate() + 1);
    }

    // Se há pelo menos 1 dia no limite máximo, exibimos o alerta preditivo
...
// [UX MELHORIA - P2] Ação interativa para preencher a data de forma rápida
...
export function handleRescueOperation() {
    ...
    // 1. Busca a carga máxima configurada pelo Perfil (8, 6 ou 4)
    const MAX_DAILY_LOAD = getDynamicLimit(appData); 

    // 2. Aplica o Fator de Carga Segura (ex: 70%) sobre o limite do perfil
...
function finalizeSave(ref, text, startDate, reviewDates) {
    // [REFATORADO] Removida a captura de mnemonic e explanation
    const newVerse = {
        id: Date.now(),
        ref: ref,
        text: text,
    ...
    // [REFATORADO] Removida a limpeza dos campos mnemônicos
    updatePreviewPanel();
...
export function startEdit(id) {
    ...
    // [REFATORADO] Array de campos simplificado
...
// [REFATORAÇÃO P3] Função centralizada para uso universal
