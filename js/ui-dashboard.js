...
import { calculateSRSDates, generateICSFile, findNextLightDay, getDynamicLimit } from './srs-engine.js';
...
export function updateRadar() {
    ...
    for (let i = 0; i < 63; i++) {
        ...
        const maxLimit = getDynamicLimit(appData);
        const safeLimit = Math.max(1, Math.floor(maxLimit * 0.5));

        if (count === 0) cell.classList.add('load-0');
        else if (count <= safeLimit) cell.classList.add('load-low');
        else if (count < maxLimit) cell.classList.add('load-med');
...
export function renderDashboard() {
    ...
    const overdueVerses = appData.verses.filter(v => {
        ...
        if (unmetDeadlines.length > 0 || isRecoveredLegacy) {
            ...
            v._isRecoveredToday = (unmetDeadlines.length === 0 && isRecoveredLegacy); 
            return true;
        }
        return false;
    ...
    if (overdueVerses.length > 0 && overduePanel) {
        ...
        if(overdueList) {
            overdueList.innerHTML = overdueVerses.map(v => {
                ...
                let badgeHTML = '';
                if (hasBadge) {
                    let badgeClass = 'interaction-badge pulse';
...
window.applySafeDate = function(isoDate) {
    const section = document.getElementById('inputSection');
...
export function handleRescueOperation() {
    ...
    const liveLoadMap = getCurrentLoadMap();

    let searchDateObj = new Date();
    searchDateObj.setDate(searchDateObj.getDate() + 1);

    const MAX_DAILY_LOAD = getDynamicLimit(appData); 
...
export function processAndGenerate() {
    ...
    if (btn && btn.classList.contains('is-blocked')) {
        ...
        // Pega a mensagem gerada pela Concordância Dupla
        const reason = btn.dataset.blockReason || "Respeite o intervalo do ciclo.";
...
export function startEdit(id) {
    ...
    const formFields = ['ref', 'startDate', 'text'];
...
export function updatePacingUI() {
    ...
    // --- GUARDA 1: O RADAR DE CAPACIDADE (Futuro) ---
    const todayStr = getLocalDateISO(new Date());
    const projectedDates = calculateSRSDates(todayStr);
    const limit = getDynamicLimit(appData);
    const loadMap = getCurrentLoadMap();
    
    const isRadarSafe = !projectedDates.some(d => (loadMap[d] || 0) >= limit);

    // --- GUARDA 2: O PERFIL DE RITMO (Cadência) ---
    let addedToday = 0;
    ...
    const yesterdayStr = getLocalDateISO(yesterdayObj);

    // Conta quantos versículos foram INICIADOS hoje e ontem
    appData.verses.forEach(v => {
        if (v.startDate === todayStr) addedToday++;
        if (v.startDate === yesterdayStr) addedYesterday++;
    });

    let isCadenceSafe = false;
    let blockReason = ""; 

    if (interval === 1) { 
        // INTENSO: Até 2 por dia
        isCadenceSafe = addedToday < 2;
        if (!isCadenceSafe) blockReason = "Você já atingiu o limite de 2 versículos hoje (Plano Intenso).";
    } 
    else if (interval === 2) { 
        // MODERADO: Até 1 por dia
        isCadenceSafe = addedToday < 1;
        if (!isCadenceSafe) blockReason = "Você já inseriu 1 versículo hoje (Plano Moderado).";
    } 
    else if (interval === 3) { 
        // LEVE: Dia sim, dia não (0 hoje E 0 ontem)
        isCadenceSafe = (addedToday === 0 && addedYesterday === 0);
        if (!isCadenceSafe) blockReason = "Dia de descanso. Retorne amanhã (Plano Leve).";
    }

    // --- CONCORDÂNCIA DUPLA ---
    if (!isRadarSafe) {
        setPacingState(btn, 'blocked');
        btn.dataset.blockReason = "Radar sobrecarregado. Priorize as revisões pendentes.";
    } 
    else if (!isCadenceSafe) {
        setPacingState(btn, 'blocked');
...
export function openChangelog() {
    ...
    if (body && window.neuroChangelog) {
        body.innerHTML = window.neuroChangelog.map(log => `
            <div class="changelog-item">
                <h4>v${log.version} <small style="opacity:0.6; font-size:0.8rem;">${log.date.split('-').reverse().join('/')}</small></h4>
                <p style="font-weight:bold; margin-bottom:10px;">${log.title}</p>
                <ul>
...
export function filterHistory(event) {
...
export function confirmSmartReschedule() {
    if (!pendingVerseData.value) return;
    ...
    data.dates.forEach(d => {
        let searchDate = d;
        while ((loadMap[searchDate] || 0) >= limit) {
            let dateObj = new Date(searchDate + 'T00:00:00');
            dateObj.setDate(dateObj.getDate() + 1);
            searchDate = getLocalDateISO(dateObj); 
        }
        optimizedDates.push(searchDate);
        loadMap[searchDate] = (loadMap[searchDate] || 0) + 1; 
    });
