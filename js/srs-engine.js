...
// Função para retornar o limite dinâmico com base no perfil do usuário (8, 6 ou 4)
export function getDynamicLimit(appData) {
    const plan = appData.settings?.planInterval || 1; 
    // Perfil: 1 (Intenso) = 8, 2 (Equilibrado) = 6, 3 (Leve) = 4
    const baseLimits = { 1: 8, 2: 6, 3: 4 };
    return baseLimits[plan] || 8; 
}

// Função unificada para mapear a carga de revisões global
export function getGlobalLoadMap(appData) {
    const loadMap = {};
    if (!appData || !appData.verses) return loadMap;
    
    appData.verses.forEach(v => {
        if (v.dates && Array.isArray(v.dates)) {
            v.dates.forEach(d => {
...
// Algoritmo Recursivo unificado para achar dia livre (Smart Reschedule)
export function findNextLightDay(dateStr, appData, maxDays = 30) {
    const limit = getDynamicLimit(appData);
    const loadMap = getGlobalLoadMap(appData);
...
// Adaptação da função para diluição de carga pesada aproveitando a função unificada
export function findNextLightDayDynamic(dateStr, appData) {
    // Reutiliza a lógica principal expandindo a janela de busca para 60 dias
    return findNextLightDay(dateStr, appData, 60);
}
